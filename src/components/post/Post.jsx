...
export default function Post({post,Users}) {
  const [like,setLike] = useState(parseInt(post.like,10));
  ...
  return (
   <div className="post">
    ...
    <div className="postTop">
      <div className="postTopLeft">
        <img className='postProfileImage' src={Users.filter((u)=>u.id==post.userId).map((u)=>u.avatar)} alt="" />
        <span className="postUsername">{Users.filter((u)=>u.id==post.userId).map((u)=>u.name)}</span>
